...
//FuncReturnFeedback_e RV64Core::VAddrRead_CoreDebugAPI(uint64_t start_addr, uint64_t size, uint8_t *buffer) {
...
//}
//
//FuncReturnFeedback_e RV64Core::VAddrWrite_CoreDebugAPI(uint64_t start_addr, uint64_t size, const uint8_t *buffer) {
...
FuncReturnFeedback_e RV64Core::MemRead_CoreDebugAPI(uint64_t start_addr, uint64_t size, uint8_t *buffer) {
    if(currentPrivMode == M_MODE || satp.mode == 0) {
        LOG_DEBUG("MMU not enabled, currently reading PADDR");
        return sv39MMU->getSysBus_Debug()->PAddr_ReadBuffer_MMIOBus_API(start_addr, size, buffer);
    }
...
FuncReturnFeedback_e RV64Core::MemWrite_CoreDebugAPI(uint64_t start_addr, uint64_t size, const uint8_t *buffer) {
    if(currentPrivMode == M_MODE || satp.mode == 0) {
...
FuncReturnFeedback_e RV64Core::WriteAllRegister_CoreAPI(const std::vector<RegisterItem_t> &regs) {
