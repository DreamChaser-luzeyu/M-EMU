            ...
            case OPCODE_AMO: {
                ...
                switch (funct5) {
                    case AMOLR: {
                        ...
                        if (inst.r_type.rs2 != 0) is_instr_illegal = true;
                            ...
                            if (inst.r_type.funct3 == 0b011) {
                                ...
                                int64_t result;
                                RV64_ExecFeedbackCode_e exc = sv39MMU->vaddrLoadReserved(GPR[inst.r_type.rs1],
                    ...
                    case AMOSC: {
                        ...
                        bool result;
                        RV64_ExecFeedbackCode_e exc = sv39MMU->vaddrStoreConditional(GPR[inst.r_type.rs1],
                                                                                     (1<<inst.r_type.funct3),
...
//                            cause.interrupt = 0;
                    ...
                    case AMOSWAP: case AMOADD: case AMOXOR: case AMOAND: case AMOOR: case AMOMIN: case AMOMAX: case AMOMINU: case AMOMAXU: {
                        ...
                        int64_t result;
                        RV64_ExecFeedbackCode_e exc = sv39MMU->vaddrAtomicMemOperation(GPR[inst.r_type.rs1],
                                                                                       (1<<inst.r_type.funct3),
            ...
            case OPCODE_SYSTEM: {
                switch (inst.i_type.funct3) {
                    ...
                    case FUNCT3_CSRRW: {
                        ...
                        uint64_t csr_result;
                        if (!is_instr_illegal) is_instr_illegal = !csr_read(csr_index, csr_result);
                        if (!is_instr_illegal) is_instr_illegal = !csr_write(csr_index, GPR[inst.i_type.rs1]);
                    ...
                    case FUNCT3_CSRRS: {
                        ...
                        uint64_t csr_result;
                        if (!is_instr_illegal) is_instr_illegal = !csr_read(csr_index, csr_result);
                        if (!is_instr_illegal && inst.i_type.rs1) is_instr_illegal = !csr_write(csr_index, csr_result | GPR[inst.i_type.rs1]);
                        if (!is_instr_illegal && inst.i_type.rd) setGPR(inst.i_type.rd, csr_result);
                        break;
                    }
                    case FUNCT3_CSRRC: {
                    ...
                    case FUNCT3_CSRRWI: {
                        ...
                        uint64_t csr_result;
                        if (!is_instr_illegal) is_instr_illegal = !csr_read(csr_index, csr_result);
                        if (!is_instr_illegal) is_instr_illegal = !csr_write(csr_index, inst.i_type.rs1);
                        if (!is_instr_illegal && inst.i_type.rd) setGPR(inst.i_type.rd, csr_result);
                        break;
                    }
                    case FUNCT3_CSRRSI: {
                        ...
                        uint64_t csr_result;
                        if (!is_instr_illegal) is_instr_illegal = !csr_read(csr_index, csr_result);
                        if (!is_instr_illegal && inst.i_type.rs1) is_instr_illegal = !csr_write(csr_index, csr_result | inst.i_type.rs1);
                        if (!is_instr_illegal && inst.i_type.rd) setGPR(inst.i_type.rd, csr_result);
                        break;
                    }
                    case FUNCT3_CSRRCI: {
