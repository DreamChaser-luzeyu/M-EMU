...
void GDBStub::cmd_ReadAllRegister() {
    ...
    debuggedCore->DumpRegister_CoreAPI(regs);
...
void GDBStub::cmd_WriteAllRegister(const GDB_RSP_Packet &packet) {
    ...
    if((packet.paramList.at(0).length() % (debuggedCore->GetISABitLen() / 4)) != 0) {
        // Send empty packet & return
        packingAndSend("", 0);
        return;
    }
    ...
    int reg_str_len = (debuggedCore->GetISABitLen() / 4);
    int reg_num = (packet.paramList.at(0).length()) / reg_str_len;
    int reg_size = debuggedCore->GetISABitLen() / 8;
    ...
    FuncReturnFeedback_e feedback = debuggedCore->WriteAllRegister_CoreAPI(regs);
    if(feedback != MEMU_OK) {
        const char* err = "E01";
...
void GDBStub::cmd_MemRead(const GDB_RSP_Packet &packet) {
    ...
    FuncReturnFeedback_e feedback = debuggedCore->MemRead_CoreDebugAPI(addr, len, data);
    // --- Encode data & send
    if(feedback == MEMU_OK) {
...
void GDBStub::cmd_MemWrite(const GDB_RSP_Packet &packet) {
    ...
    // --- Write data to mem
    FuncReturnFeedback_e feedback = debuggedCore->MemWrite_CoreDebugAPI(addr, len, data);
    if(feedback != MEMU_OK) {
        const char* err = "E01";
        packingAndSend(err, strlen(err));
    }
...
void GDBStub::cmd_InsertBreakpoint(const GDB_RSP_Packet &packet) {
    ...
    debuggedCore->InsertWatchPoint_CoreDebug({ .u64_val = addr });
...
void GDBStub::cmd_RemoveBreakpoint(const GDB_RSP_Packet &packet) {
    ...
    uint64_t addr = strtoll(packet.paramList.at(1).c_str(), nullptr, 16);
    debuggedCore->RemoveWatchPoint_CoreDebug({ .u64_val = addr });
...
void GDBStub::cmd_Continue(const GDB_RSP_Packet &packet) {
    ...
    while(!debuggedCore->IsCurrentPCAtWatchPoint_CoreDebug()) {
        debuggedCore->Step_CoreAPI();
//        usleep(100 * 1000);
    }
